...
class MOBILAgent(Agent):
    ...
    def __init__(
        ...
        dt: float = 0.1,
        config: MobilParams = MobilParams(),
    ):
        ...
        self.clcs_main = self.dict_clcs_ego[main_lanelet_id][
            self.dict_list_lanelets_merged_ego[main_lanelet_id][0].lanelet_id
        ]
    ...
    def step_forward(self, time_step: int):
        ...
        if not state_new:
            ...
            shape_new = self.shape.rotate_translate_local(
            ...
            self.set_ids_lanelets_current = set(
                self.cls_scenario_current.lanelet_network.find_lanelet_by_shape(
                    self.occ_current.shape
                )
            )
    ...
    def _calculate_new_state(self, time_step: int):
        try:
            ...
            if self.id_follower:
                obs_follower_old_current = self.cls_scenario_current.obstacle_by_id(
                    self.id_follower
                )
                follower_old_state_current = self.state_at_step(
                    time_step - 1, obs_follower_old_current
                )

                (
                    p_lon_follower_old_current,
                    p_lat_follower_old_current,
                    v_lon_follower_old_current,
                    v_lat_follower_old_current,
                    o_ref_follower_old_current,
                ) = self._convert_to_curvilinear_state(
                    follower_old_state_current, self.ego_clcs
                )

            a_follower_old_current = self._calculate_acceleration(
                self.id_follower,
                v_lon_follower_old_current,
                self.id_agent,
            ...
            a_follower_old_change = self._calculate_acceleration(
                self.id_follower,
                v_lon_follower_old_current,
                self.id_leader,
                self.dist_to_leader_min + self.dist_to_follower_min,
                self.rate_approaching_leader + self.rate_approaching_follower,
            )

            if self.left_clcs:
                v_lon_follower_left_current = 0
                if self.id_left_follower:
                    obs_follower_left_current = (
                        self.cls_scenario_current.obstacle_by_id(self.id_left_follower)
                    )
                    follower_left_state_current = self.state_at_step(
                        time_step - 1, obs_follower_left_current
                    )

                    (
                        p_lon_follower_left_current,
                        p_lat_follower_left_current,
                        v_lon_follower_left_current,
                        v_lat_follower_left_current,
                        o_ref_follower_left_current,
                    ) = self._convert_to_curvilinear_state(
                        follower_left_state_current, self.left_clcs
                    )

                a_follower_left_current = self._calculate_acceleration(
                    self.id_left_follower,
                    v_lon_follower_left_current,
                    self.id_left_leader,
                    self.dist_to_left_leader_min + self.dist_to_left_follower_min,
                ...
                (
                    ...
                    ego_state_current, self.left_clcs
                )
                ...
                a_follower_left_change = self._calculate_acceleration(
                    self.id_left_follower,
                    v_lon_follower_left_current,
                    self.id_agent,
                ...
                a_total_change_left = (
                    ...
                    + self.p
                    * (
                        a_follower_left_change
                        - a_follower_left_current
                        + a_follower_old_change
                        - a_follower_old_current
                    )
                )
            else:
            ...
            if self.right_clcs:
                ...
                if self.id_right_follower:
                    obs_follower_right_current = (
                        self.cls_scenario_current.obstacle_by_id(self.id_right_follower)
                    )
                    follower_right_state_current = self.state_at_step(
                        time_step - 1, obs_follower_right_current
                    )

                    (
                        p_lon_follower_right_current,
                        p_lat_follower_right_current,
                        v_lon_follower_right_current,
                        v_lat_follower_right_current,
                        o_ref_follower_right_current,
                    ) = self._convert_to_curvilinear_state(
                        follower_right_state_current, self.right_clcs
                    )

                a_follower_right_current = self._calculate_acceleration(
                    self.id_right_follower,
                    v_lon_follower_right_current,
                    self.id_right_leader,
                    self.dist_to_right_leader_min + self.dist_to_right_follower_min,
                ...
                (
                    ...
                    ego_state_current, self.right_clcs
                )
                ...
                a_follower_right_change = self._calculate_acceleration(
                    self.id_right_follower,
                    v_lon_follower_right_current,
                    self.id_agent,
                ...
                a_total_change_right = (
                    a_ego_change_right
                    - a_ego_current
                    + self.p
                    * (
                        a_follower_right_change
                        - a_follower_right_current
                        + a_follower_old_change
                        - a_follower_old_current
                    )
                )
            ...
            if (
                ...
                and a_total_change_left > self.a_th
                and a_follower_left_change >= -self.b_safe
            ):
                # change to left lane
                ...
                a_total_change_right > self.a_th
                and a_follower_right_change >= -self.b_safe
            ):
            ...
            p_lat_new = p_lat_current + (
                lateral_coeff * np.sign(p_lat_current)
                if abs(p_lat_current) > 0.2
                else 0
            )
            ...
            x_new, y_new = self.clcs_main.convert_to_cartesian_coords(
                p_lon_new, p_lat_new
            )
    ...
    def _update_followers_and_leaders_at_time_step(self, time_step: int):
        ...
        for lanelet_id in self.set_ids_lanelets_current_pred_removed:
            ...
            ) = self._find_followers_and_leaders_at_time_step(
                time_step,
                self.dict_list_lanelets_merged_ego[lanelet_id],
                self.dict_clcs_ego[lanelet_id],
                self.dict_lanelet_merge_ids_ego[lanelet_id],
                state_ego,
            )
            if not ego_clcs:
                ego_clcs = self.dict_clcs_ego[lanelet_id][
                    self.dict_list_lanelets_merged_ego[lanelet_id][0].lanelet_id
                ]
        ...
        if not self.left_clcs and len(self.list_lanelets_merged_left) > 0:
            self.left_clcs = self.dict_clcs_left[
                self.list_lanelets_merged_left[0].lanelet_id
            ]

        if not self.right_clcs and len(self.list_lanelets_merged_right) > 0:
            self.right_clcs = self.dict_clcs_right[
                self.list_lanelets_merged_right[0].lanelet_id
            ]
    ...
    def _calculate_acceleration(
        ...
        if leader_id:
            ...
            term_3 = (v_lon_current * rate_approach) / (
                2 * np.sqrt(self.a_lon_max * self.a_lon_comfort)
            )
            a_interact = (
                -self.a_lon_max * ((term_1 + term_2 + term_3) / dis_to_leader) ** 2
            )
    ...
    def _retrieve_merged_lanelet_ids_and_create_clcs(self):
        ...
        for id_lanelet_current in self.set_ids_lanelets_current:
            lanelet_current = (
                self.cls_scenario_current.lanelet_network.find_lanelet_by_id(
                    id_lanelet_current
                )
            )
            ...
            if lanelet_current.adj_left is not None:
                if (
                    ...
                    and lanelet_current.adj_left not in self.set_ids_lanelets_current
                ):
    ...
    def _get_merged_lanelet_and_clcs(self, lanelet_id: int):
        if lanelet_id in self.dict_all_merged_lanelets_containing_id:
            list_ids_lanelets_merged = self.dict_all_merged_lanelets_containing_id[
                lanelet_id
            ]
            ...
            for merged_id in list_ids_lanelets_merged:
                list_lanelets_merged.append(
                    self.dict_all_merged_lanelets[merged_id]["lanelet"]
                )
                dict_clcs[merged_id] = self.dict_all_merged_lanelets[merged_id]["CLCS"]
                dict_merge_ids[merged_id] = self.dict_all_merged_lanelets[merged_id][
                    "merges"
                ]
        else:
            lnlt = self.cls_scenario_current.lanelet_network.find_lanelet_by_id(
                lanelet_id
            )
            list_lanelets_merged, dict_clcs, dict_merge_ids = (
                self._merge_lanelet_and_create_clcs(lnlt)
            )
            ...
            for lanelet in list_lanelets_merged:
                if lanelet.lanelet_id in self.dict_all_merged_lanelets:
                    ...
                    self.dict_all_merged_lanelets[lanelet.lanelet_id]["lanelet"] = (
                        lanelet
                    )
                    self.dict_all_merged_lanelets[lanelet.lanelet_id]["CLCS"] = (
                        dict_clcs[lanelet.lanelet_id]
                    )
                    self.dict_all_merged_lanelets[lanelet.lanelet_id]["merges"] = (
                        dict_merge_ids[lanelet.lanelet_id]
                    )

                    for partial_id in dict_merge_ids[lanelet.lanelet_id]:
                        if (
                            partial_id
                            not in self.dict_all_merged_lanelets_containing_id
                        ):
                            self.dict_all_merged_lanelets_containing_id[partial_id] = (
                                set()
                            )
                        self.dict_all_merged_lanelets_containing_id[partial_id].add(
                            lanelet.lanelet_id
                        )
