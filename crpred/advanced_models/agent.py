...
class Agent:
    ...
    """
    cls_scenario_current: Scenario = None

    def __init__(self, obstacle: DynamicObstacle):
        self.obstacle: DynamicObstacle = obstacle
        self.valid = True

        self.id_agent = obstacle.obstacle_id
        self.shape = obstacle.obstacle_shape

        self.dt: float = self.cls_scenario_current.dt
        self.state_initial = obstacle.initial_state
        self.time_step_initial = self.state_initial.time_step
        self.state_current = copy.deepcopy(self.state_initial)
        self.time_step_current = self.state_current.time_step
        self.occ_current = obstacle.occupancy_at_time(self.time_step_initial)
    ...
    def _retrieve_merged_lanelets(self, lanelet: Lanelet, merge_predecessors: bool = True):
        list_lanelets_merged_suc, list_suc_merge_ids = \
            Lanelet.all_lanelets_by_merging_successors_from_lanelet(
                lanelet=lanelet,
                network=self.cls_scenario_current.lanelet_network,
                max_length=300.0)
            ...
            for lnlt, suc_merge_ids in zip(list_lanelets_merged_suc, list_suc_merge_ids):
                list_lanelets_merged_pred, list_pred_merge_ids = \
                    all_lanelets_by_merging_predecessors_from_lanelet(lnlt,
                                                                      self.cls_scenario_current.lanelet_network,
                                                                      300.0)
    ...
    def _find_followers_and_leaders_at_time_step(self, time_step: int, list_lanelets: List[Lanelet],
                                                 ...
                                                 dict_merge_ids: Dict[int, Set[int]], state_ego: State):
        ...
        for lanelet in list_lanelets:
            ...
            for id_obs in list_ids_obstacles_in_lanelet:
                obs_candidate = self.cls_scenario_current.obstacle_by_id(id_obs)
        ...
        if id_follower:
            # calculate the approaching rate
            obs_follow = self.cls_scenario_current.obstacle_by_id(id_follower)
            obs_lead = obs_ego
    ...
    def _dynamic_obstacles_in_lanelet_set(self, set_lanelet_ids: Set[int], time_step: int):
        ...
        for l_id in set_lanelet_ids:
            lanelet = self.cls_scenario_current.lanelet_network.find_lanelet_by_id(l_id)
            if lanelet:
    ...
    @staticmethod
    def _convert_to_curvilinear_state(state: State, clcs: CurvilinearCoordinateSystem):
        x, y = state.position
        o = state.orientation
        v = state.velocity
    ...
    @staticmethod
    def state_at_step(step: int, obstacle: DynamicObstacle):
        if step < obstacle.initial_state.time_step:
            ...
            return obstacle.prediction.trajectory.state_list[step - obstacle.initial_state.time_step - 1]
