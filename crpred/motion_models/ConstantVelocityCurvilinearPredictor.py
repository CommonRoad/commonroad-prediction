...
class ConstantVelocityCurvilinearPredictor(MotionModelInterface):
    """"
    A curveilinear, constant Velocity motion model
    ...
    def state_prediction_generator(self, state_list: List[State]) -> List[State]:
        """
        Next states are generated assuming no acceleration but the turn rate of the vehicle is included
        the avg_orientation_delta is the constant turn rate of a CTRV Motion model
        """
        ...
        avg_orientation_delta: float = mean(_difference([s.orientation for s in state_list]))
        ...
        v_lon: float = v * np.cos(orientation - np.interp(p_lon, ref_path_length, ref_orientation))
        v_lat: float = v * np.sin(orientation - np.interp(p_lon, ref_path_length, ref_orientation))
