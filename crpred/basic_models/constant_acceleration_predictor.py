...
class ConstantAccelerationLinearPredictor(MotionModelPredictor):
    ...
    def _predict_states(
        ...
        for t in prediction_range:
            p_lon = (
                p_lon
                + np.cos(initial_values.orientation_in_ccosy)
                * (v_lon + 0.5 * initial_values.acceleration * dt)
                * dt
            )
            p_lat = p_lat + v_lat * dt

            v_lon = (
                v_lon + initial_values.acceleration * dt
            )  # acceleration only in longitudinal direction
...
class ConstantAccelerationCurvilinearPredictor(MotionModelPredictor):
    ...
    def _predict_states(
        ...
        for t in prediction_range:
            ...
            v_lon = (
                prev_state_v * np.cos(orientation_in_ccosy)
                + initial_values.acceleration * dt
            )
