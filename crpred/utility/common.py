...
def clear_obstacle_trajectory(sc: Scenario, initial_time_step: int = 0):
    ...
    for obstacle in sc.dynamic_obstacles:
        ...
        for time_step in range(
            initial_time_step + 1, len(obstacle.prediction.trajectory.state_list) + 1
        ):
            for lanelet in sc.lanelet_network.lanelets:
                try:
                    lanelet.dynamic_obstacles_on_lanelet.get(time_step).discard(
...
def get_merged_laneletes_from_position(
    lanelet_network: LaneletNetwork, position: np.ndarray
):
    ...
    merged_lanelets, merged_lanelets_id = (
